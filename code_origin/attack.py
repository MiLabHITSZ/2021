...
def set_params_init(params, values, num_param_to_set=60):
    ...
    if not isinstance(values, np.ndarray):  # 判断类别是否一致 ndarray可以随机生成数组
        values = values.get_value()  # ----

    params_to_set = []
    for p in params:
        if p.ndim > 1:  # 判断p是一个大于等于二维数组
    ...
    for p in params_to_set:
        ...
        n = np.prod(shape)  # 将数组维数相乘，得到每一个params的参数个数
...
def corr_term(params, targets, size=None):
    ...
    if isinstance(params, list):
        params = T.concatenate([T.flatten(p) for p in params if p.ndim > 1])  # 对参数进行拼接

    if size is not None:
        targets = targets[:size]
        params = params[:size]  # 将params和targets剪切成相同数量的向量
...
def sign_term(params, targets, size):
    ...
    penalty = T.switch(T.gt(constraints, 0), 0, constraints)  # gt是将每个元素逐步比较 返回true或者false，switch是根据前面条件取结果
    penalty = abs(penalty)
...
def mal_data_synthesis(train_x, num_targets=10, precision=4):
    ...
    num_targets /= 2  # 算出可以窃取的像素数
    if num_targets == 0:
        num_targets = 1

    targets = train_x[:num_targets]  # 截取出要窃取的数据像素点个数
